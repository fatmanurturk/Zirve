...
import { Plus, Users, Calendar, MapPin, Inbox, ArrowRight, Trash2 } from "lucide-react";
            ...
            {events.map((event) => (
              ...
              >
                ...
                {confirmDeleteId === event.id ? (
                  <div className="flex items-center justify-between gap-4 px-6 py-4 bg-red-50 rounded-2xl border border-red-200">
                    <p className="text-sm font-semibold text-red-700">
                      <strong>&quot;{event.title}&quot;</strong> silinecek. Bu işlem geri alınamaz.
                    </p>
                  ...
                  <div
                    ...
                    className="p-6 hover:border-green-200 hover:bg-green-50/30 transition-all flex flex-col md:flex-row md:items-center justify-between gap-6 group cursor-pointer rounded-2xl"
                  >
                    ...
                    <div className="flex items-center gap-3 md:border-l md:border-gray-100 md:pl-8">
                      <div className="hidden md:block text-right pr-2">
                        <p className="text-sm text-gray-700 font-bold mb-0.5">Başvurular</p>
                        <p className="text-xs text-green-700 font-bold group-hover:translate-x-1 transition-transform inline-block">Yönet & İncele →</p>
                      </div>
                      <button className="w-full md:w-auto px-6 py-3 bg-gray-50 hover:bg-gray-100 text-gray-700 text-sm font-bold rounded-xl transition-colors border border-gray-100 shadow-sm whitespace-nowrap">
                        Detay Gör
                      </button>
                      <button
                        onClick={(e) => { e.stopPropagation(); setConfirmDeleteId(event.id); }}
                        className="p-3 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-colors border border-gray-100 flex-shrink-0"
                        title="Etkinliği Sil"
                      >
